...
const Navbar: React.FC<NavbarProps> = ({ toggleSidebar }) => {
  ...
  return (
    <nav className="bg-white/80 backdrop-blur-md border-b border-slate-200 p-4 sticky top-0 z-30 shadow-sm">
        ...
        <div className="hidden lg:flex flex-1 max-w-md mx-8">
          <div className="relative w-full">
            ...
            <input 
              ...
              className="w-full bg-slate-100/50 border border-slate-200 rounded-full py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-rose-400 focus:bg-white transition-all"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-col items-end leading-none hidden xs:flex">
              <span className="text-sm font-bold text-slate-900 mb-1">{user?.username}</span>
              {renderRoleBadge()}
            </div>
            ...
            <div className="group relative">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-rose-500 to-orange-500 p-[2px] cursor-pointer shadow-lg shadow-rose-500/20">
                <div className="w-full h-full bg-white rounded-[10px] flex items-center justify-center text-rose-500 font-bold">
                  {user?.username?.charAt(0).toUpperCase()}
                </div>
              </div>
              
              <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-200 rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 overflow-hidden">
                ...
                <Link 
                  to="/profile"
                  className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-rose-500 transition-all border-b border-slate-50"
                >
                  <User size={18} />
                  My Profile
                </Link>
                <button 
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-600 hover:bg-rose-50 hover:text-rose-500 transition-all"
                >
                  <LogOut size={18} />
                  Sign Out
                </button>
