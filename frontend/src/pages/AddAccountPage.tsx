    ...
    <div className="max-w-2xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-1000 pb-20 px-4 sm:px-0 text-slate-900 dark:text-white selection:bg-cyan-500/30">
      ...
      <div className="bg-white dark:bg-[#0a0a0a] border border-slate-200 dark:border-white/10 rounded-[2.5rem] md:rounded-[3rem] shadow-xl dark:shadow-2xl overflow-hidden relative group hover:border-slate-300 dark:hover:border-white/20 transition-all duration-500">
        <div className="bg-slate-50 dark:bg-white/5 p-8 md:p-10 border-b border-slate-200 dark:border-white/5 relative overflow-hidden text-slate-900 dark:text-white">
           ...
           <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-600 dark:text-cyan-400 text-[8px] md:text-[9px] font-bold tracking-widest uppercase mb-4 relative z-10">
             <Activity size={10} /> Add User
           </div>
           <h2 className="text-3xl md:text-4xl font-black tracking-tighter relative z-10 uppercase text-slate-900 dark:text-white">New Account<span className="text-cyan-400">.</span></h2>
        ...
        {success ? (
          <div className="p-16 md:p-24 flex flex-col items-center justify-center text-center space-y-6">
             <div className="w-16 h-16 md:w-20 md:h-20 bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 rounded-full flex items-center justify-center border border-cyan-500/30 shadow-sm dark:shadow-[0_0_30px_rgba(34,211,238,0.2)] animate-in zoom-in duration-500">
                <CheckCircle2 className="w-[32px] h-[32px] md:w-[40px] md:h-[40px]" />
             </div>
             <div className="space-y-2">
               <h3 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase">User Created!</h3>
          ...
          <form onSubmit={handleSubmit} className="p-8 md:p-10 space-y-6 md:space-y-8">
            ...
            <div className="space-y-3">
              ...
              <div className="relative group/input">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl md:rounded-2xl py-3.5 md:py-4 pl-12 md:pl-14 pr-4 focus:ring-1 focus:ring-cyan-500/50 outline-none transition-all font-bold text-xs uppercase tracking-widest text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-700 shadow-sm"
            ...
            <div className="space-y-3">
              ...
              <div className="relative group/input">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl md:rounded-2xl py-3.5 md:py-4 pl-12 md:pl-14 pr-6 focus:ring-1 focus:ring-cyan-500/50 outline-none transition-all font-bold text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-700 shadow-sm"
                 ...
                 {['VIP', 'Free'].map((role) => (
                    <button
                       ...
                       className={`py-3 md:py-4 rounded-xl md:rounded-2xl border-2 font-black text-[10px] md:text-xs uppercase tracking-widest transition-all ${
                          formData.role === role 
                             ? 'border-cyan-500 bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 shadow-sm dark:shadow-[0_0_20px_rgba(34,211,238,0.15)]' 
                             : 'border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 text-slate-500 hover:border-slate-200 dark:hover:border-white/10 dark:hover:text-slate-300 shadow-sm'
                       }`}
                    >
                       {role} Level
                    </button>
            ...
            <div className="bg-slate-50 dark:bg-white/5 p-4 md:p-5 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm">
               <p className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Security Note</p>
               <p className="text-[10px] md:text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed">Default password is set to <span className="text-cyan-600 dark:text-cyan-400 font-bold tracking-widest">123</span>. New users must reset their password upon first login.</p>
            </div>

            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full bg-slate-900 dark:bg-white text-white dark:text-black py-4 md:py-5 rounded-[1.5rem] md:rounded-[2rem] font-black text-[11px] md:text-sm uppercase tracking-[0.3em] shadow-lg dark:shadow-[0_0_30px_rgba(255,255,255,0.2)] hover:shadow-xl dark:hover:shadow-[0_0_50px_rgba(255,255,255,0.4)] hover:scale-[1.01] active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-70 mt-2 md:mt-4"
            >
              {isSubmitting ? (
                <>
