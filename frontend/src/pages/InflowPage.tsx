...
const InflowPage: React.FC = () => {
  ...
  const fetchTransactions = async () => {
    setIsFetching(true);
    try {
      let url = `${import.meta.env.VITE_API_URL || 'http://localhost:8800'}/transactions?type=Income`;
    ...
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-1000 pb-24 md:pb-32 px-4 sm:px-0 text-slate-800 dark:text-white selection:bg-cyan-500/10 selection:text-cyan-700 dark:selection:text-cyan-200">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 relative z-20 pt-4 md:pt-8">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          <div className="relative group w-full md:w-64">
            <span className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-cyan-600 transition-colors pointer-events-none">
              <WalletIcon size={16} />
            </span>
            <select
              value={selectedAccountId}
              onChange={(e) => setSelectedAccountId(e.target.value)}
              className="w-full bg-white/80 dark:bg-white/5 border border-white/50 dark:border-white/10 rounded-2xl py-3.5 md:py-4 pl-12 pr-6 focus:ring-1 focus:ring-cyan-500/30 outline-none transition-all text-[10px] md:text-xs font-black tracking-widest uppercase text-slate-800 dark:text-white shadow-sm dark:shadow-2xl backdrop-blur-md appearance-none cursor-pointer"
            >
      ...
      <div className="bg-white/70 backdrop-blur-2xl dark:bg-[#0a0a0a] border border-white/50 dark:border-white/10 rounded-[2.5rem] md:rounded-[3rem] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-2xl relative z-20">
        ...
        <div className="md:hidden space-y-4 p-4 sm:p-6">
          {filteredTransactions.length > 0 ? filteredTransactions.map((t) => (
            <div key={t.id} className="bg-white/70 dark:bg-white/[0.03] p-4 rounded-2xl flex justify-between items-center border border-white dark:border-white/10 shadow-sm">
              <div className="flex items-center gap-4 overflow-hidden">
                <div className="w-10 h-10 bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 rounded-xl flex items-center justify-center font-black text-base border border-cyan-500/10 shrink-0">
                  {t.category.charAt(0)}
                </div>
                <div className="truncate">
                  <h4 className="font-black text-sm text-slate-800 dark:text-white uppercase truncate">{t.category}</h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{t.description || 'N/A'}</p>
                </div>
              </div>
              <div className="text-right shrink-0 ml-4">
                <p className="font-black text-base text-cyan-600 dark:text-cyan-400 whitespace-nowrap">{formatCurrency(t.amount)}</p>
