  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-1000 pb-24 md:pb-32 px-4 sm:px-0 text-white selection:bg-cyan-500/30">
      ...
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none z-[-1] overflow-hidden bg-[#030303]">
        <div className="absolute bottom-[-20%] right-[-10%] w-[50%] h-[50%] bg-violet-600/10 blur-[150px] rounded-full mix-blend-screen" />
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 relative z-20 pt-4 md:pt-8">
        ...
        <div className="relative group w-full md:w-96">
          ...
          <input 
            ...
            className="w-full bg-white/5 border border-white/10 rounded-2xl py-3.5 md:py-4 pl-12 md:pl-14 pr-6 focus:ring-1 focus:ring-violet-500/50 outline-none transition-all text-[10px] md:text-xs font-black tracking-widest uppercase placeholder:text-slate-600 dark:text-white shadow-2xl backdrop-blur-md"
          />
        </div>
      </div>
                ...
                <tr key={t.id} className="group hover:bg-white/[0.03] transition-all cursor-pointer">
                  <td className="px-6 md:px-10 py-5 md:py-6">
                    <div className="flex items-center gap-3 md:gap-5">
                      <div className="w-10 md:w-12 h-10 md:h-12 bg-violet-500/10 text-violet-400 rounded-xl md:rounded-2xl flex items-center justify-center font-black text-base md:text-lg border border-violet-500/20 shadow-[0_0_15px_rgba(139,92,246,0.1)] group-hover:scale-110 transition-transform shrink-0">
                        {t.category.charAt(0)}
                      </div>
                  ...
                  <td className="px-6 md:px-10 py-5 md:py-6 text-right">
                    <div className="flex items-center justify-end gap-2 md:gap-2.5">
                      <ArrowDownCircle className="w-[14px] h-[14px] md:w-[16px] md:h-[16px] text-violet-400 shrink-0" />
                      <span className="font-black text-sm md:text-lg text-violet-400 tracking-tighter whitespace-nowrap">{formatCurrency(t.amount)}</span>
                    </div>
                  </td>
                </tr>
              )) : (
